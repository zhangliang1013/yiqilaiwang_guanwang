		...
		methods: {
			...
			getCode() {
				...
				this.http.ajax({
					...
					// method: 'post',
					url: 'login/mobile/' + this.telPhone,
					method: 'get',
					// headers: null,
					// data: {
					// 	type: this.type,
			...
			getToken(){
				...
				this.http.ajax({
					...
					if (res.code == 200) {
						this.local.save('mount', res.data.token);
					}
				})
			}
		}
