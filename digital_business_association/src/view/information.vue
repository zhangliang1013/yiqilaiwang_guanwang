		...
		<div class="w bg" style="margin-bottom: 64px;">
			<div class="contentaudit" >
				...
				<div class="u">
					...
					<span class="time">{{ new Date(createTime).Format("yyyy-MM-dd") }}</span>				
				</div>
					...
					<div class="mc" v-for="(value,index) in fileCollection" :key="index" style="padding:0;flex-direction:column;">
						<div class="item" style="display:flex;height:93px;border-bottom:1px solid #EEEEEE;padding-left:20px">
							...
							<div style="padding:18px 15px 0 0; margin:0px;display:inline-block;width: 860px;font-size:16px">
								<p style="line-height:30px;margin:  0;margin-bottom:7px">{{type==15?value.file:value.name}} <span style="color:#ccc">&nbsp;（{{type==15?value.fileSize:value.size}}k）</span></p>
								...
								<p style="line-height:30px;margin:  0;">
									<a href="javascript:void(0)" @click="download(type==15?value.fileUrl:value.url,type==15?value.file:value.name)">下载</a> 
									&nbsp;&nbsp;&nbsp;
									<p v-if="type!=15">
									...
									<p v-if="type==15">
										<a target='_black' v-if="value.file.indexOf('pdf') == -1&& value.file.indexOf('txt') == -1" :href="'https://view.officeapps.live.com/op/view.aspx?src='+value.fileUrl">预览</a>
										<span v-else @click="tolayer(1,value)" style="color: #2D8cF0;;cursor: pointer;">预览</span>
									</p>
								</p>
							</div>
				...
				<div class="bens " v-else>
					<div class="btn1 hand" v-on:click="back" style="width:206px">返回上一页</div>
					<!-- <div class="btn2 hand" v-on:click="save(1)">发布</div> -->
				</div>
			</div>
		</div>
...
<script>
	...
	export default {
		...
		methods: {
			...
			downloadAll(){
				if(this.type==15){
					...
					this.download(this.fileCollection[index].fileUrl,this.fileCollection[index].fileName)
				}
				}else{
					for(var index in this.fileCollection){
				...
				this.http.ajax({
					...
					.then(res => {
						...
						this.activities = res
							...
							this.type = res.data.messageType
		...
		created() {
			...
			if(this.$route.query.messageType==3){
				this.getOrgMessageListDeatil1();
			...
			// console.log('fgdgy',this.$route.query.isAudit,this.$route.query.messageType);
		}
	}
</script>
