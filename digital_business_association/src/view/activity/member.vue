				...
				<div class="table">
					...
					<div class="mc" v-for="(item,index) in list" :key="index">
						...
						<div class="item hand" style="text-align: right;" >
							<span style="color: #2168eb;" v-if="item.isCheck==0" @click="audit(item)">待审核</span>
							<span style="color: #5AA20C;" v-if="item.isCheck==1" @click="message()">已通过</span>
							<span style="color: #DC1A00;" v-if="item.isCheck==2" @click="message()">不通过</span>
						</div>
					</div>
...
<script>
	export default {
		...
		methods: {
			...
			let _t = this;
				layer.confirm('您确认要允许<span style="color:red;">' + value.groupNickname + '</span>参与该活动吗??', {
					title: ['活动审核'],
					closeBtn: false,
					btnAlign: 'c',
					shadeClose: true,
			...
			auditMethod(value,type){
				this.http.ajax({
					url: '../../webSite/act/participation/review',
					method: 'post',
					headers: {
