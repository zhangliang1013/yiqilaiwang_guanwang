<template>
	<div>
		<div class="w">
			<div class="position">{{detail&&detail.isCreate ==1?'活动管理':'活动详情'}}
				<span ><router-link :to="'/activitylists?page='+page" >返回</router-link></span>
			</div>
		</div>
		<div class="w bg" style="margin-bottom: 30px;">
			<div class="activitydetails" v-if="detail">
				<div class="title">{{detail.topic}}
					<div class="btn" :style="{'background':(detail.status==0? '#25CC9D':(detail.status==1?'#2168EB':'#999999'))}">
						{{detail.status==-1?'草稿':detail.status==0?'报名中':detail.status==1?'已开始':detail.status==2?'已结束':detail.status==3?'已取消':detail.status==4?'报名已截止':''}}
					</div>
				...
				<div class="line"></div>
				<div class="guanli" v-if='detail&&detail.isCreate ==1'>
					<div class="t">
						活动管理
					</div>
					
					<div class="m " v-if="detail.status !=-1" style="position:relative;" v-bind:class="nopointer">
						<router-link :to="'/activityrelease?id='+detail.id" class="item" v-if=" detail&&detail.status !=2 && detail.status !=3&& detail.status !=1 && detail.isCreate ==1">
							<div class="pic">
								<img src="../../assets/img/hd6@2x.png" alt="">
							</div>
							<p>修改活动</p>
						</router-link>
						<div class="item grayscale" v-if="detail&&(detail.status == 2 ||detail.status == 3||detail.status == 1 )&& detail.isCreate ==1">
							<div class="pic">
								<img src="../../assets/img/hd6@2x.png" alt="">
							</div>
							<p>修改活动</p>
						</div>
						<div class="item hand" @click="tolayer(1)" v-if="detail&&detail.status !=2 && detail.status !=3 && detail.status !=1 && detail.isCreate ==1">
							<div class="pic">
						...
						<div class="item grayscale hand" v-if="detail&&(detail.status ==2 ||detail.status ==3 || detail.status ==1 ) && detail.isCreate ==1">
							<div class="pic">
								<img src="../../assets/img/hd7@2x.png" alt="">
							</div>
							<p>取消活动</p>
						</div>
						<router-link :to="'/activitymember?id='+detail.id" class="item" v-if="detail&&detail.status !=2 &&detail.status !=3 && detail.isCreate ==1">
						...
						<div class="item grayscale" v-if="detail&&(detail.status ==2 ||detail.status ==3 ) && detail.isCreate ==1">
							<div class="pic">
								<img src="../../assets/img/hd8@2x.png" alt="">
							</div>
							<p>活动审核</p>
						...
						<!-- <router-link :to="'/activitycost?id='+detail.id+'&status='+detail.status" class="item" v-if="detail.isCreate ==1">
							<div class="pic">
				...
				<div class="baoming" v-if="list&&list.rows">
					...
					<div class="tips">邀请、评论功能请去一起来往手机客户端进一步操作，活动位于手机客户端的“我的——活动”</div>
				</div>
				<!--//活动审核按钮-->
				<!-- <div class="bens">
