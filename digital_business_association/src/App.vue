<template>
	<div id="app" :style="{'background':$route.name=='echarts'?'#0C0B34':'#f7f7f7'}">
		...
		<router-view v-if='isAlive' style="min-height:800px" :login="login" :personalInfo.sync="personalInfo" :defOrg="defOrg" @onlogin="onlogin"
		@getdefOrg="getdefOrg" @tokenTimeOut="tokenTimeOut" @signout="onsignout" />

		<footers v-if="$route.name!='echarts'"></footers>
	</div>
</template>
...
<script>
	export default {
		...
		methods: {
			...
			getdefOrg(id) {
				...
				this.http.ajax({
					...
					if (res.code == 200) {
						...
						this.local.save('defOrg', res.data);
			...
			onsignout() {
				this.local.save('tokeng', null);
				this.local.save('mount', null);
				this.local.save('loging', null);
				this.local.save('currentOrgNameg', null);
				this.personalInfo = {
					avatarUrl: '',
					id: null,
				}
			...
			onlogin(login) {
				this.login = login;
				if (this.login) {
					this.getdefOrg(this.local.fetch('defOrg')?this.local.fetch('defOrg').digitalCommerce:login.relations[0].entityId)
				}
		...
		created() {
			this.onlogin(this.local.fetch('loging'));
		}
	}
</script>
