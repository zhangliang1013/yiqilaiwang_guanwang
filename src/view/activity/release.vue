		...
		<div class="w bg">
			<div class="release" v-if="formdate">
				<Form :label-width="100" :model="formdate" :rules="formdaterule" ref="formdaterule">
					...
					<FormItem label="选择组织" v-if="formdate.actClass!=1">
					...
					<FormItem label="活动简介">
						...
						<div ref="editor" style="width:150%;text-align:left;position: relative;z-index: 0;word-break: break-all;" v-model="content" @click="getToken($event)"></div>
					...
					<div class="input_label">
						<div class="mt">点击选择需要填写的信息 <div class="btn" @click="onisfieldsedit">{{isfieldsedit?'保存':'编辑'}}</div>
						</div>
						<div class="mc">
					...
					<FormItem label="邀请人">
						<Select v-model="formdate.parts" multiple placeholder="所有成员" :class="{ 'edit' :  disabledState }">
							<Option :value="item.value" v-for="(item,index) in StructureList" :key="index" :disabled="item.disabled">{{item.structure}}</Option>
						</Select>
					</FormItem>
					<div class="switch">
					...
					<div class="line"></div>
					<div class="xy hand">
						<Checkbox v-model="isxy"> 我同意</Checkbox> 《<span @click="pop('https://www.yiqilaiwang.com/law/serviceagree.html','一起来往使用条款')">一起来往使用条款</span>》
					</div>
	...
	export default {
		...
		methods: {
						...
						for(i in this.StructureList){
							...
							for(j in this.oldParts){
								if(this.StructureList[i].id == this.oldParts[j]){
									...
									// console.log('index',index,this.StructureList[i].disabled)
								}
							}
						}
					}
			...
			onform() {
				...
				// console.log(this.formdate.chooseConsume)
				...
				// console.log("-----------------------")
				// console.log(this.formdate.perUpperLimit)
				// console.log(num)
				// console.log("-----------------------")
				...
				// var list=[]
				// this.fileCollection.forEach((v, i) => {
				// 
				// 	list.push(v[1])
				// })
				this.formdate.posterUrl = this.fileCollection
				// console.log(this.formdate.actType)
				if (this.formdate.chooseConsume == 1) {
					// console.log(this.formdate.perCapita)
					// console.log(this.formdate.perCapita == "")
					// console.log(this.formdate.perCapita, this.formdate.perCapita == "" || this.formdate.perCapita == 0)
			...
			getactivityDetail() {
				...
				this.http.ajax({
					...
					if (res.code == 200) {
						...
						this.oldParts = this.formdate.parts;
						// console.log('this.formdate',res.data,this.formdate.parts)
						this.content = res.data.activityProcess
						if (res.data.chooseConsume != null && res.data.chooseConsume != '') {
							// console.log(res.data.chooseConsume)
							// console.log("res.data.chooseConsume")
						...
						// var that = this;
						// console.log('454745', res)
						//获取邀请人列表
						this.getStructureList();
						

						// this.http.ajax({
						// 	url: 'webSite/activity/getStructureDetail',
						// 	method: 'post',
						// 	headers: {
						// 		'Content-Type': ' application/json'
						// 	},
		...
		created() {
			...
			if (this.defOrg) {
				...
				this.formdate.orgId = this.orgId
				this.getOrgList();
				if(!this.$route.query.id){
					this.getStructureList();
				}
