<template>
	<div>
		<div class="w">
			<div class="position">政策详情
				<span ><router-link :to="'/policyList?page='+page + '&messageType=1'"  >返回</router-link></span>
			</div>
		</div>
		<div class="w bg" style="margin-bottom: 64px;">
			<div class="contentaudit" >
				<div class="title"><span>政策 • </span>{{title}}
				</div>
				<div class="u">
					...
					<div class="mc" v-for="(value,index) in fileCollection" :key="index" style="padding:0;flex-direction:column;">
						<div class="item" style="display:flex;height:93px;border-bottom:1px solid #EEEEEE;padding-left:20px">
							...
							<div style="padding:18px 15px 0 0; margin:0px;display:inline-block;width: 860px;font-size:16px">
								<p style="line-height:30px;margin:  0;margin-bottom:7px">{{ value.name }} <span style="color:#ccc">&nbsp;（{{value.inx}}k）</span></p>
...
<script>
	...
	export default {
		...
		methods: {
			...
			downloadAll(){		
				for(var item of this.fileCollection){
				...
				this.http.ajax({
					...
					.then(res => {
						...
						if (res.code == 200) {
							...
							this.orgName = res.data.creatorName;
							this.orgUrl = res.data.entity.name;
							this.createTime =new Date(res.data.createdDate).Format("yyyy-MM-dd");
							this.fileCollection = res.data.attachments
